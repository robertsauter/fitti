...
class TrainingDataParser {
    constructor() {
        ...
        this.parseGlobalExercise = this.parseGlobalExercise.bind(this);
    ...
    getTrainingData(trainingData) {
        ...
        if ('globalExercises' in trainingData) {
            const parsedGlobalExercises = this.#parseGlobalExercises(trainingData.globalExercises);

            if (parsedGlobalExercises !== null) {
                parsedTrainingData.globalExercises = parsedGlobalExercises;
            }
        }
    ...
    /** 
     * @param {unknown} exercises  
     * @returns {ExerciseResponse[] | null}
     * */
    #parseGlobalExercises(exercises) {
        if (!Array.isArray(exercises)) {
            return null;
        }

        return exercises
            .map(this.parseGlobalExercise)
            .filter((exercise) => exercise !== null);
    }
    ...
    parseExerciseHistory(history) {
        ...
        if (!('ExerciseId' in history) || typeof history.ExerciseId !== 'string') {
            return null;
        }
    ...
    /** 
     * @param {unknown} exercises  
     * @returns {WorkoutStartExerxise[] | null}
     * */
